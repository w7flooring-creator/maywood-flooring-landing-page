...
**2026-06-10 增量**：#10/#11 产品迁移完成（73 产品 + 图片入 Sanity）；#59 Wix 视觉对齐 69/69 关闭
（双分类 `extraCategories`、画廊 32 图差异化 alt、ATFA 徽标、T&C PDF 平迁等）；#25 表单后端上线
（见「表单与获客」）；Sanity publish webhook → CF Deploy Hook 已配（发布即自动重建）。

**仍待办（HITL）**：
- **#73**：DNS 迁移到 Cloudflare + Resend 域名验证（**#27 前置**；完成后表单收件切回 `sales@`）。
- **#27**：上线 QA + Lighthouse + DNS 切换（图片优化是 Lighthouse 关键项）。
...
- **Phase 2 ✅ 已上线（2026-06-10）**：同 Worker 加 routes（`worker/index.ts`：`POST /api/contact`、
  `POST /api/sample`）+ Turnstile 防垃圾 + Resend 发信；服务端复用 zod schema，不存任何数据。
  ⚠️ **当前为过渡态（Resend 域名验证被 Wix DNS 阻塞，见 #73）**：发件人 `onboarding@resend.dev`、
  收件 `LEADS_TO_EMAIL=w7flooring@gmail.com`（Resend 未验证域只允许发给账号邮箱）。
  #73 完成后切回 `sales@maywoodflooring.com.au` 并启用 `CONTACT_FROM_EMAIL`。
- **Phase 3**：CRM、自动报价、样品追踪、预约。

**环境变量**（不要提交 secrets）：
`PUBLIC_SANITY_PROJECT_ID` `PUBLIC_SANITY_DATASET` `SANITY_API_READ_TOKEN`(build 时拉取内容必需，见 ADR-0003) `RESEND_API_KEY` `LEADS_TO_EMAIL` `TURNSTILE_SECRET_KEY` `PUBLIC_TURNSTILE_SITE_KEY`(⚠️ 放 Workers **Build** 变量，build 时进前端 bundle) `CONTACT_FROM_EMAIL`(可选，待 #73 域名验证后启用)
...
- **#73 DNS 迁移到 Cloudflare + Resend 域名验证**（#27 前置，与站主一起做）：域名注册在 GoDaddy、
  NS 现指 Wix（`ns8/9.wixdns.net`）；Wix DNS 不支持子域 MX → Resend 无法验证。迁 CF 时必须
  原样平移 Google Workspace MX×5 + SPF（`sales@` 收信）与 Wix 网站记录。完成后：Resend 验证
  `maywoodflooring.com.au` → `LEADS_TO_EMAIL` 切回 `sales@` + 启用 `CONTACT_FROM_EMAIL`。
  红利：#27 的 DNS 切换简化为 CF 内改记录（秒级，无 NS 传播等待）。详见 issue #73。
